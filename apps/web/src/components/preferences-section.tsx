...
const controlClass =
  'min-w-32 rounded-md border border-border bg-background px-3 py-2 text-sm font-medium outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-foreground';
...
export function PreferencesSection() {
  ...
  useEffect(() => {
    ...
    const followSystemTheme = () =>
      applyPreferences(preferences, document.documentElement, colorScheme.matches);
  ...
  return (
    <section aria-labelledby="preferences-heading" className="border-t border-border">
      <div className="py-6 sm:py-8">
        ...
        <h2 id="preferences-heading" className="font-serif text-2xl tracking-[-0.02em] sm:text-3xl">
          Preferences
        </h2>
      </div>
      ...
      <div className="divide-y divide-border border-y border-border">
        <div className="flex min-h-24 flex-col items-stretch justify-between gap-3 py-5 sm:flex-row sm:items-center sm:gap-6">
          ...
          <div
            ...
            className="flex shrink-0 rounded-md border border-border bg-background p-1"
          >
        ...
        <label className="flex min-h-24 flex-col items-stretch justify-between gap-3 py-5 sm:flex-row sm:items-center sm:gap-6">
          ...
          <select
            ...
            onChange={(event) =>
              updatePreference('appLanguage', event.target.value as Preferences['appLanguage'])
            }
            className={`${controlClass} w-full sm:w-auto`}
          >
            {APP_PREFERENCE_LANGUAGES.map((language) => (
        ...
        <label className="flex min-h-24 flex-col items-stretch justify-between gap-3 py-5 sm:flex-row sm:items-center sm:gap-6">
          <span>
            ...
            <span className="mt-1 block text-sm leading-6 text-muted-foreground">
              Adjust the body text size in the reader.
            </span>
          </span>
          <select
      ...
      {storageError && (
        <p role="alert" className="mt-4 text-sm text-(--color-rose)">
          {storageError}
        </p>
      )}
