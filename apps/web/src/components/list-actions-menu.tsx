...
export function ListActionsMenu({ actions }: { actions: Action[] }) {
  ...
  return (
    <div ref={root} className="relative shrink-0" onBlur={(event) => {
      ...
      <button ref={trigger} type="button" aria-label="List actions" aria-haspopup="menu" aria-expanded={open} aria-controls={open ? id : undefined}
      ...
      {open && <div id={id} role="menu" aria-label="List actions" className="absolute right-0 top-full z-20 mt-2 w-52 max-w-[calc(100vw-64px)] rounded-lg border border-border bg-paper p-1 shadow-sm" onKeyDown={(event) => {
        ...
        {actions.map((action, index) => <button key={action.label} ref={(element) => { items.current[index] = element; }} type="button" role="menuitem" tabIndex={-1}
          onClick={() => { setOpen(false); trigger.current?.focus(); action.onSelect(); }}
