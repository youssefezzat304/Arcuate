...
export function SavedWordList({ id }: { id: string }) {
  ...
  useEffect(() => {
    function load() {
      try { setState({ status: "ready", list: readWordList(id) }); }
  ...
  if (state.status === "error") return <p role="alert">This list could not be opened. It may have been deleted, saved in another browser, or browser storage may be unavailable.</p>;
