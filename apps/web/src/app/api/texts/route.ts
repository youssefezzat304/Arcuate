...
export async function POST(request: Request) {
  ...
  if (!parsed.success)
    return Response.json(
      { error: 'Check the topic, language, level, and length.' },
      { status: 400 },
    );
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey)
    return Response.json(
      { error: 'Set GEMINI_API_KEY in apps/web/.env.local to generate texts.' },
      { status: 503 },
    );
    ...
    return Response.json(
      {
        error:
          error instanceof GenerationError
            ? error.message
            : 'Unable to generate a valid text. Please try again.',
      },
