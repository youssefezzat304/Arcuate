...
test("provider sends level and character target and validates structured output", async (t) => {
  ...
  t.mock.method(globalThis,"fetch",async (request, init) => {
    ...
    return new Response(JSON.stringify({candidates:[{finishReason:"STOP",content:{role:"model",parts:[{text:JSON.stringify({title:record.title,paragraphs:record.paragraphs})}]}}]}),{status:200,headers:{"Content-Type":"application/json"}});
  });
  const result = await generateText({topic:settings.topic,language:"German",level:"A2",characterTarget:2000},{apiKey:"test-key",model:"gemini-3.6-flash"});
...
test("provider quota errors are safe and actionable", async (t) => {
  t.mock.method(globalThis,"fetch",async()=>new Response(JSON.stringify({error:{code:429,message:"private provider details",status:"RESOURCE_EXHAUSTED"}}),{status:429,headers:{"Content-Type":"application/json"}}));
  await assert.rejects(generateText({topic:"Sun",language:"German",level:"A2",characterTarget:2000},{apiKey:"test-key",model:"gemini-3.6-flash"}), (error)=>error.status===429 && !error.message.includes("private provider details"));
});

test("provider rejects malformed and incomplete responses", async (t) => {
  for (const [status, text] of [["completed", "not json"], ["completed", '{"title":"Empty","paragraphs":[]}'], ["incomplete", '{"title":"Cut off","paragraphs":["unfinished"]}']]) {
    const mock = t.mock.method(globalThis, "fetch", async () => new Response(JSON.stringify({
      candidates: [{ finishReason: status === "completed" ? "STOP" : "MAX_TOKENS", content: { role: "model", parts: [{ text }] } }],
    }), { headers: { "Content-Type": "application/json" } }));
    await assert.rejects(generateText({ topic: "Sun", language: "German", level: "A2", characterTarget: 2000 }, { apiKey: "test-key", model: "gemini-3.6-flash" }));
    mock.mock.restore();
  }
});
