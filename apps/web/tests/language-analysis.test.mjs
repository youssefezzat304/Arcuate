...
import {
  ...
  analyzeTextExact,
  normalizeToken,
} from '../../../packages/language/src/index.ts';
...
test('exact analysis creates locale-aware word offsets and sentence context', () => {
  ...
  assert.equal(
    analysis.paragraphs[0].map((token) => paragraph.slice(token.start, token.end)).join('|'),
    'Ein|Hund|Zwei|Hunde',
  );
  assert.deepEqual(
    analysis.paragraphs[0].map((token) => token.sentenceIndex),
    [0, 0, 1, 1],
  );
...
test('exact matching respects complete token boundaries', () => {
  ...
  assert.deepEqual(
    findSavedWordRanges(paragraph, tokens, saved, 'de').map(({ start, end }) =>
...
test('lemma matching highlights inflected forms and remains language-specific', () => {
  ...
  const saved = [
    { ...savedBase, text: 'Hund', normalizedText: 'hund', lemmas: ['Hund'], partOfSpeech: 'NOUN' },
  ];
