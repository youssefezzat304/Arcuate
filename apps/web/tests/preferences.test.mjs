...
test("missing or invalid preferences safely use defaults", () => {
  ...
  assert.deepEqual(
    readPreferences(createStorage(JSON.stringify({ theme: "midnight", appLanguage: "xx", readerFontSize: 48 }))),
    DEFAULT_PREFERENCES,
  );
});
...
test("preferences are applied to the document root", () => {
  ...
  const root = { dataset: {}, lang: "", style: { setProperty(key, value) { properties.set(key, value); } } };
...
test("legacy display preferences migrate to the new model", () => {
  const storage = createStorage(JSON.stringify({ darkMode: true, appLanguage: "en", fontSize: "medium" }));

  assert.deepEqual(readPreferences(storage), { theme: "dark", appLanguage: "en", readerFontSize: 14 });
});
